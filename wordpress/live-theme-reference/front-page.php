...
if (!function_exists('sanatcin_home_hero_image_profile')) {
  function sanatcin_home_hero_image_profile($post_id) {
    ...
    $eligible = $width >= 1400 && $ratio >= 1.35 && $ratio <= 1.90;
    // An explicit flag must never override the physical image requirements.
...
// Publication day controls freshness on SanatÇin, not the source's calendar day.
usort($home_pool, function($a, $b) {
  $day_delta = strcmp(get_post_time('Y-m-d', true, $b->ID), get_post_time('Y-m-d', true, $a->ID));
  if ($day_delta !== 0) return $day_delta;
  $delta = sanatcin_home_rank_score($b->ID) <=> sanatcin_home_rank_score($a->ID);
  return $delta !== 0 ? $delta : (get_post_time('U', true, $b->ID) <=> get_post_time('U', true, $a->ID));
});

// Only eligible photographs can become a visual lead. Fall back to an older
// eligible story in the existing seven-day pool; never relax image requirements.
$hero_id = 0;
foreach ($home_pool as $candidate) {
  if (sanatcin_home_hero_image_profile($candidate->ID)['eligible']) {
    $hero_id = (int) $candidate->ID;
    break;
  }
}
$hero = new WP_Query(array('post__in'=>array($hero_id ?: 0),'posts_per_page'=>1,'post_status'=>'publish'));

$home_card_ids = array();
$home_category_counts = array();
$latest_home_day = $home_pool ? get_post_time('Y-m-d', true, $home_pool[0]->ID) : '';
// Today's publications get visibility first; then fill missing categories
// before taking a second older story from a represented category.
for ($pass = 0; $pass < 3; $pass++) {
  foreach ($home_pool as $candidate) {
    $candidate_id = (int) $candidate->ID;
    if ($candidate_id === $hero_id || in_array($candidate_id, $home_card_ids, true)) continue;
    $categories = get_the_category($candidate_id);
    $primary_slug = $categories ? $categories[0]->slug : 'uncategorized';
    $count = $home_category_counts[$primary_slug] ?? 0;
    if ($count >= 2) continue;
    if ($pass === 0 && get_post_time('Y-m-d', true, $candidate_id) !== $latest_home_day) continue;
    if ($pass === 1 && $count > 0) continue;
    $home_card_ids[] = $candidate_id;
    $home_category_counts[$primary_slug] = $count + 1;
    if (count($home_card_ids) >= 5) break 2;
  }
}
