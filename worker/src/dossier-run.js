...
async function run() {
  ...
  try {
    ...
    const article = await writeDossierArticle(verified, { signal: controller.signal });
    ...
    if (mode === 'preview') {
      log('info', 'Preview modu: WordPress taslağı oluşturulmadı', {
        ...
        title: article.title,
        wordCount: article.wordCount,
        images: images.map((image) => ({ title: image.title, license: image.license, source: image.sourcePage }))
      });
      return;
    }
